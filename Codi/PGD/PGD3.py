...
for k in range(30):  # we could choose more iterations
    # compute IIk2
...
VVk1 = np.dot(Yinv, IIk1 + I0_pq)
VVk2 = np.dot(Yinv, IIk2 + I0_pq)
VVk3 = np.dot(Yinv, IIk3 + I0_pq)
VVk4 = np.dot(Yinv, IIk4 + I0_pq)
...
IIk1 = np.random.rand(102)  # try this, np.conj(VVk1)
IIp1 = np.random.rand(102)
IIq1 = np.random.rand(1001)
...
# try to initialize randomnly also
IIk2 = VVk2 
IIp2 = np.random.rand(102)
IIq2 = np.random.rand(1001)
...
Vfinal1 = VVk1 * IIp1[50] * IIq1[1000] + VVk2 * IIp2[50] * IIq2[1000] + VVk3 * IIp3[50] * IIq3[1000] + VVk4 * IIp4[50] * IIq4[1000]
# print(Vfinal1 + np.dot(Yinv, I0_pq))
    
# -----------------END OUTER LOOP NUMBER 2-----------------


# -----------------BEGIN OUTER LOOP NUMBER n-----------------
# do many outer loops
