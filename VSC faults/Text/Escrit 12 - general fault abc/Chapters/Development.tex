...
\label{eq:nconv}
\end{equation}
where matrices of the form $\mathbf{\un{Y'}_x}$ differ from $\mathbf{\un{Y'}_x}$ in the sense that they are not $3\times3$ matrices but instead are constituted by null elements except for the ones that symbolize the connection between a generic bus with the associated $\un{V}_{px}$ voltage and the grid. 
