...
In theory, the voltages $V^+$ and $V^-$ are continously measured. The injected current depends on it, but it certainly has an effect on that as well. Thus, we expect to not follow a one-step procedure, but to update iteratively the injected current until we reach a converging state. If we denote the measures as $V^+_m$, $V^-_m$ and $V^+$, $V^-$ the updated voltages due to the injection of current from the VSC, we ought to follow Algorithm \ref{alg:1}. 
...
\begin{figure}[ht]
  ...
  \begin{minipage}{.7\linewidth}
    \begin{algorithm}[H]
      \SetAlgoLined
      \KwData{Current max($\underline{I}_{abc}$), voltages $V^+_m$, $V^-_m$ and tolerance $tol_2$}
  ...
  \end{minipage}
\end{figure}
The $tol_2$ parameter will take an arbitrary value, such as $1\cdot 10^{-4}$, for example. It is used to ensure that we operate close to the maximum current while not exceeding it.