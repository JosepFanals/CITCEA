  ...
  \begin{subfigure}[!htb]{.4\textwidth}
    \centering
        \begin{tikzpicture}[trim axis right,trim axis left]
            ...
            \begin{axis}[grid=major, xlabel={${I}^+_{re}$}, ylabel={$f$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick={-1,-0.5,...,1}, ytick={0.1,0.2,...,1}, scatter/classes={a={mark=o,draw=black, mark size=1pt}, b={mark=x,draw=red, mark size=2pt}, c={mark=square,draw=orange, mark size=1.5pt}},  scatter src=explicit symbolic, axis line style = thick, legend style={at={(1.03,-0.03)},anchor=north west}]
...
\begin{subfigure}[!htb]{.4\textwidth}
    \centering
        \begin{tikzpicture}[trim axis right,trim axis left]
            \pgfplotsset{width=7cm, height=6cm}
            \begin{axis}[grid=major, xlabel={${I}^+_{im}$}, ylabel={$f$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10},xtick={-1,-0.5,...,1}, ytick={0.1,0.2,...,1}, scatter/classes={a={mark=o,draw=black, mark size=1pt}, b={mark=x,draw=red, mark size=2pt}, c={mark=square,draw=orange, mark size=1.5pt}},  scatter src=explicit symbolic, axis line style = thick, legend style={at={(0.97,0.03)},anchor=south east}]
...
The line to ground fault has the particularity of presenting a distribution of dots similar for both real and imaginary currents for both sequences. As shown in Figure \ref{fig:LGx1}, the minimum in the real positive sequence current plot takes place around the zero. Despite that, the real negative sequence current becomes significantly larger than zero. This justifies the difference between the objective functions. Notice that the negative sequence real current is not precisely on top of the brute force computations. As explained, smaller intervals wuld cause it to be between the intervals. It is also curious to realize that the real negative sequence current takes positive values. While somewhat counter-intuitive, it is the best choice in order to minimize the objective function. However, it takes already extremely small values.
...
\begin{subfigure}[!htb]{.4\textwidth}
  ...
  \label{fig:LLx1}
\end{figure}
However, this time it has been checked that there is only one point that minimizes the function, independently of the initialization vector. From what we have been able to test, when the fault becomes rather weak, multiple optimal points are present. The results also suggest that the fault impedance ought to vary in order to simulate similar faults in terms of severity. This will be done in the analysis of the submarine cable and the varying $R/X$ ratio. However, for the line to line fault we are studying, another relevant factor is the fact that best choice would be to inject a close to one real negative sequence current (in absolute value). The ROPT compensates for this limiation by injecting more reactive positive sequence current. 
...
As we will see later on, when dealing with the double line to ground fault, there is hardly any room for improvement. It does not matter much if we are in the OPT or in the ROPT case, or even the value of $\underline{Z}_f$. The thing is that since there is a null impedance between the two faulted phases, the double line to ground fault turns out to be a solid line to line fault. There is not much that can be done by the converter in such situation.
...
\begin{figure}[!htb]\centering \footnotesize
  \begin{subfigure}[!htb]{.4\textwidth}
    \centering
        \begin{tikzpicture}[trim axis right,trim axis left]
            \pgfplotsset{width=7cm, height=6cm}
            \begin{axis}[grid=major, xlabel={${I}^+_{re}$}, ylabel={$f$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick={-1,-0.5,...,1}, ytick={0.9,0.95,...,1.1}, scatter/classes={a={mark=o,draw=black, mark size=1pt}, b={mark=x,draw=red, mark size=2pt}, c={mark=square,draw=orange, mark size=1.5pt}},  scatter src=explicit symbolic, axis line style = thick, legend style={at={(1.03,-0.03)},anchor=north west}]
...
\begin{subfigure}[!htb]{.4\textwidth}
    \centering
        \begin{tikzpicture}[trim axis right,trim axis left]
            \pgfplotsset{width=7cm, height=6cm}
            \begin{axis}[grid=major, xlabel={${I}^+_{im}$}, ylabel={$f$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10},xtick={-1,-0.5,...,1}, ytick={0.9,0.95,...,1.1}, scatter/classes={a={mark=o,draw=black, mark size=1pt}, b={mark=x,draw=red, mark size=2pt}, c={mark=square,draw=orange, mark size=1.5pt}},  scatter src=explicit symbolic, axis line style = thick, legend style={at={(0.97,0.03)},anchor=south east}]
...
\begin{table}[!htb] \centering
    \begin{tabular}{lccccccc}
       ...
       \hline
    \end{tabular}
    \caption{Main results for the balanced and the unbalanced faults in the OPT case, with $\lambda^+=1$, $\lambda^-=1$}
    \label{tab:results}
\end{table}