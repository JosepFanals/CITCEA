...
\begin{equation}
    ...
    \label{eq:1}
\end{equation}
where $\lambda=[0,1]$. Therefore, bigger values of $\lambda$ would imply that we prioritize the positive sequence voltage while small values will tend to give more importance to the negative sequence voltage. Figure XX shows the voltage profiles for a sweep of $\lambda$ values. 

\begin{tikzpicture}
\begin{axis}[%
width=8cm, height=8cm, grid=major, xlabel={$|V^+_c|$}, ylabel={$|V^-_c|$}, zlabel={$\lambda$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick={0.6,0.63,...,1}, ytick={0.1,0.14,...,1}, ztick={0,0.2,...,1}, axis line style = very thick]
\addplot3[colormap/jet, surf, very thick] table[x=x, y=y, z=z, col sep=comma] {Data/constant/pareto_3x.csv};
...
\begin{axis}[%
    ...
    colorbar style={
        at={(1.1,0.3)},
        anchor=south west,
        height=0.35*\pgfkeysvalueof{/pgfplots/parent axis height},
        title={$f$}
    }
]
...
% \addplot3 [surf, name path=A] table[x=x, y=y, z=z, col sep=comma] {Data/constant/ff_3x.csv};
% \addplot3 [shading=stefan] [name path=A, line width=1pt, gray] table[x=x, y=y, z=z, col sep=comma] {Data/constant/ff_3x.csv} \closedcycle;
\addplot3 [scatter, only marks] table[x=x, y=y, z=z, col sep=comma] {Data/constant/ff_3x.csv};

% \path [name path=B] (1,0,0) -- (0,1,0);
% \addplot3 [name path=B, gray, no markers, line width=1pt] table[x=x, y=y, z=z, col sep=comma] {Data/constant/terra.csv};

\addplot3 [domain=-0:1,samples=31, samples y=0, thick, smooth]  table[x=x, y=y, z=z, col sep=comma] {Data/constant/V1_3x.csv};
\addplot3 [domain=-0:1,samples=31, samples y=0, thick, smooth]  table[x=x, y=y, z=z, col sep=comma] {Data/constant/V2_3x.csv};

% \addplot3 [color=brown!50] fill between[of = A and B];

% \draw [black!50] (axis cs:-1,0,0) -- (axis cs:4,0,0);
% \draw [black!50] (axis cs:0,-1,0) -- (axis cs:0,4,0);

% \node at (axis cs:-1,1,0.18) [pin=165:$|V^+_c|$] {};
% \node at (axis cs:1.5,4,0.32) [pin=-15:$|V^-_c|$] {};
\end{axis}
\end{tikzpicture}
