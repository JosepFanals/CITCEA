...
for kk in range(n_punts):
    ...
    def gR1(x):
        I012p = Vabc_to_012(np.array([(x[0] + 1j * x[1]) * np.exp(1j * angVa(x)), (x[2] + 1j * x[3]) * np.exp(1j * angVa(x)), (x[4] + 1j * x[5]) * np.exp(1j * angVa(x))]))
        return np.real(I012p[1])

    def gR2(x):
        I012p = Vabc_to_012(np.array([(x[0] + 1j * x[1]) * np.exp(1j * angVa(x)), (x[2] + 1j * x[3]) * np.exp(1j * angVa(x)), (x[4] + 1j * x[5]) * np.exp(1j * angVa(x))]))
        return np.real(I012p[2])

    def angVa(x):
        return np.angle(Va(x))
...
make_csv(Zc_vec, np.real(I1_vec), a_vec, 'Data/submarine/RI1_re_LG.csv')
make_csv(Zc_vec, np.imag(I1_vec), a_vec, 'Data/submarine/RI1_im_LG.csv')
make_csv(Zc_vec, np.real(I2_vec), a_vec, 'Data/submarine/RI2_re_LG.csv')
make_csv(Zc_vec, np.imag(I2_vec), a_vec, 'Data/submarine/RI2_im_LG.csv')
make_csv(Zc_vec, ff_vec, a_vec, 'Data/submarine/Rff_LG.csv')