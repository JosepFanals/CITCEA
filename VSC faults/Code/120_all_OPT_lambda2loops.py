...
for ll in range(n_punts + 1):
    lamm1 = lim2_lam + ll * increment

    for kk in range(n_punts + 1):

        lamm2 = lim2_lam + kk * increment
        lam_vec.append(lamm1)
        lam2_vec.append(lamm2)
        ...
        def objective(x):
            objective = lamm1 * abs(np.abs(V1(x)) - 1) + lamm2 * abs(np.abs(V2(x)) - 0) 
            return objective
...
make_csv(lam_vec, np.real(I1_vec), a_vec, 'Data/Mconstant/I1_re_LG.csv')
make_csv(lam_vec, np.imag(I1_vec), a_vec, 'Data/Mconstant/I1_im_LG.csv')
make_csv(lam_vec, np.real(I2_vec), a_vec, 'Data/Mconstant/I2_re_LG.csv')
make_csv(lam_vec, np.imag(I2_vec), a_vec, 'Data/Mconstant/I2_im_LG.csv')

make3_csv(lam_vec, one_vec, np.abs(V1_vec), 'Data/Mconstant/V1_3x.csv')
make3_csv(one_vec, lam2_vec, np.abs(V2_vec), 'Data/Mconstant/V2_3x.csv')
make3_csv(lam_vec, lam2_vec, ff_vec, 'Data/Mconstant/ff_3x.csv')

make3_csv(np.abs(V1_vec), np.abs(V2_vec), lam_vec, 'Data/Mconstant/pareto_3x.csv')
