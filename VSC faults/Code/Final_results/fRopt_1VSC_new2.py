...
def fROptimal2_mystic(V_mod, Imax, Zv1, Zt, Y_con, Y_gnd, lam_vec, Ii_t):
    ...
    def obj_fun(x):
        ...
        p_ff = np.real(V_p1_abc[0] * np.conj(x[0]) + V_p1_abc[1] * np.conj(x[1]) + V_p1_abc[2] * np.conj(x[2]))  # manual penalty

        Vp1 = V_p1_012[1]
        Vn1 = V_p1_012[2]

        # suma = lam_vec[0] * abs(1 - abs(Vp1)) + lam_vec[1] * abs(0 - abs(Vn1)) 
        # suma = lam_vec[0] * abs(1 - abs(Vp1)) + lam_vec[1] * abs(0 - abs(Vn1)) + 100000000 * abs(np.real(V_p1_abc[0] * np.conj(x[0]) + V_p1_abc[1] * np.conj(x[1]) + V_p1_abc[2] * np.conj(x[2]))) 
        suma = lam_vec[0] * abs(1 - abs(Vp1)) + lam_vec[1] * abs(0 - abs(Vn1))
        return suma
    ...
    sol = my.diffev(obj_fun, Ii_t, penalty=pens, disp=True, bounds=bnds, gtol=5, ftol=1e-4, full_output=True, maxiter=100000, maxfun=100000)
