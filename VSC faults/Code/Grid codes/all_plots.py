...
lim_inf = -10
lim_sup = 10
for I1_im in range(lim_inf, lim_sup, 1):
    for I2_im in range(lim_inf, lim_sup, 1):
        for I1_re in range(lim_inf, lim_sup, 1):
            for I2_re in range(lim_inf, lim_sup, 1):
                I1_im = I1_im / lim_sup
                I2_im = I2_im / lim_sup
                I1_re = I1_re / lim_sup
                I2_re = I1_re / lim_sup
                ...
                I012f = np.array([0, I1_re + 1j * I1_im, I2_re + 1j * I2_im])
                Iopt = [I1_re, I1_im, I2_re, I2_im]
                ...
                if max(Iabcf) < Imax:
                    ...
                    ff_vec.append(lam_1 * abs(1 - abs(V1_vec[-1])) + lam_2 * abs(0 - abs(V2_vec[-1])))
                print('aaaaaaaaaaa')
...
zero_vec = np.full(len(ff_vec), 0)
one_vec = np.full(len(ff_vec), 5)
a_vec = np.full(len(ff_vec), 'a')
b_vec = 'b'

make_csv(np.real(I1_vec), ff_vec, a_vec, 'Optimal/Data/constant/I1_re_3x.csv')
make_csv(np.imag(I1_vec), ff_vec, a_vec, 'Optimal/Data/constant/I1_im_3x.csv')
make_csv(np.real(I2_vec), ff_vec,  a_vec, 'Optimal/Data/constant/I2_re_3x.csv')
make_csv(np.imag(I2_vec), ff_vec, a_vec, 'Optimal/Data/constant/I2_im_3x.csv')
make_csv(ff_vec, ff_vec, a_vec, 'Optimal/Data/constant/ff_LLG.csv')

make_csv(ff_vec, np.abs(V1_vec), a_vec, 'Optimal/Data/constant/V1_LLG.csv')
make_csv(ff_vec, np.abs(V2_vec), a_vec, 'Optimal/Data/constant/V2_LLG.csv')
make_csv(ff_vec, ff_vec, a_vec, 'Optimal/Data/constant/ffG_LLG.csv')


fig, axs = plt.subplots(3,2)
fig.suptitle('Vertically and horizontally stacked subplots')
axs[0,0].plot(np.real(I1_vec), ff_vec)
axs[0,1].plot(np.imag(I1_vec), ff_vec)
axs[1,0].plot(np.real(I2_vec), ff_vec)
axs[1,1].plot(np.imag(I2_vec), ff_vec)
axs[2,0].plot(ff_vec, ff_vec)
