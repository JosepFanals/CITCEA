...
for lamm in range(0,100,1):
    lam_1 = lamm / 100
    lam_2 = 1 - lam_1 
    lam_vec.append(lam_1)
    ...
    def g4(x):
        aang_V1 = ang_V1V2(x)[0]
        return np.real((x[0] + 1j * x[1]) * np.exp(- 1j * aang_V1))

    def g5(x):
        aang_V2 = ang_V1V2(x)[1]
        return np.real((x[2] + 1j * x[3]) * np.exp(- 1j * aang_V2))

    def ang_V1V2(x):
        V012_ang = np.array([V0(x), V1(x), V2(x)])
        ang_V1 = np.angle(V012_ang[1])
        ang_V2 = np.angle(V012_ang[2])
        return [ang_V1, ang_V2]
    ...
    sol = minimize(objective, x0, method='SLSQP', bounds=bnds, constraints=cons, options={'ftol':1e-7})
    Iopt = sol.x
    I0f = 0
...
# make_csv(RX_vec, np.real(I1_vec), a_vec, 'Optimal/Data/RX/RI1_re_LLG.csv')
# make_csv(RX_vec, np.imag(I1_vec), a_vec, 'Optimal/Data/RX/RI1_im_LLG.csv')
# make_csv(RX_vec, np.real(I2_vec), a_vec, 'Optimal/Data/RX/RI2_re_LLG.csv')
# make_csv(RX_vec, np.imag(I2_vec), a_vec, 'Optimal/Data/RX/RI2_im_LLG.csv')
# make_csv(RX_vec, ff_vec, a_vec, 'Optimal/Data/RX/Rff_LLG.csv')

make_csv(lam_vec, np.abs(V1_vec), 'Optimal/Data/constant/RV1_LLG.csv')
make_csv(lam_vec, np.abs(V2_vec), 'Optimal/Data/constant/RV2_LLG.csv')
# make_csv(RX_vec, ff_vec, a_vec, 'Optimal/Data/RX/RffG_LLG.csv')
