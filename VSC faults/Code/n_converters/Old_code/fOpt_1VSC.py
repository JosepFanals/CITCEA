...
from mystic.solvers import fmin, fmin_powell, diffev, diffev2, sparsity, buckshot, lattice
...
def fOptimal_mystic(V_mod, Imax, Zv1, Zt, Y_con, Y_gnd, lam_vec, Ii_t):
    ...
    def constraint_Imax(x):
        ...
        I1b2 = x[2] ** 2 + x[3] ** 2
        I1c2 = x[4] ** 2 + x[5] ** 2
        return Imax ** 2 - max(I1a2, I1b2, I1c2)
    ...
    def objective_f(x):
        ...
        suma = lam_vec[0] * (1 - abs(Vp1)) + lam_vec[1] * (0 + abs(Vn1)) 
        return suma
    ...
    def obj_fun(x):
        ...
        suma = np.real(lam_vec[0] * (1 - Vp1 * np.conj(Vp1)) ** 2 + lam_vec[1] * (0 + Vn1 * np.conj(Vn1)) ** 2)
        return suma
    ...
    result = diffev(cost=obj_fun, x0=Ii_t, bounds=bnds, penalty=penalty, constraints=cf, npop=100, gtol=100, disp=True, full_output=True, ftol=1e-15, maxiter=350000, maxfun=350000, scale=0.9, cross=0.9)
