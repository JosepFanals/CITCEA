...
\label{eq:nconv}
\end{equation}
where vector currents of the form $\mathbf{\un{I}_k} = \left[\un{I}^{(a)}_k, \un{I}^{(b)}_k, \un{I}^{(c)}_k \right]^T$, i.e., they contain the $a$, $b$ and $c$ phase currents. The same applies to the voltages $\mathbf{\un{V}_{pk}} = \left[\un{V}^{(a)}_{pk}, \un{V}^{(b)}_{pk}, \un{V}^{(c)}_{pk} \right]^T$. These variables are related via admittance submatrices as $\mathbf{\un{Y}_{vk}}$, which represent the connection between the point of common coupling PCCk and the corresponding node of the grid. 
