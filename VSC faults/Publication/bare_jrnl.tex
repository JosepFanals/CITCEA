...
Figure \ref{fig:sys_n} depicts a generic system formed by $n$ converters where the goal is to optimize the voltages at the point of common coupling. VSCs are typically modeled as three-phase voltage sources in series with the filters impedance of the form $\underline{Z}_z$. The control strategy directly adjusts the voltages so that currents can indirectly meet the references \cite{andani2018controller}. However, the approach followed here consists of treating them as a current sources that inject a current of the form $\underline{I}_k$ into the system, i.e, into the grid denoted by $\mathbf{\underline{Y}_g}$. Such currents are precisely the variables to control in order to improve the voltages $\underline{V}_p$. 
...
\label{eq:nconv}
\end{equation}
where matrices of the form $\mathbf{\underline{Y}_{vk}}$ stand for the connection between the point of common coupling of bus $k$ and the grid, whereas $\mathbf{\underline{Y}_g}$ changes depending on the fault characteristics.  Each specific voltage and current object is composed by three elements, one related to each phase. Voltages are expressed as a transformation of currents by computing the inverse of the general admittance matrix. 

In order to have a clearer comprehension of the voltages, they are converted into symmetrical components by means of Fortescue's transformation\cite{fortescue1918method}:
\begin{equation}
  \begin{pmatrix}
    \un{V}^{0}_{pk} \\
    \un{V}^{+}_{pk} \\
  ...
  \end{pmatrix}\ ,
\end{equation}
where $a=e^{j\frac{2\pi}{3}}$. The zero sequence component of the voltages is a magnitude likely to become non-null under asymmetrical faults. Nevertheless, as VSC are unable to inject zero sequence currents, it will remain an uncontrolled variable in the sense that no efforts will be made towards reducing it. On the contrary, positive sequence voltages have to be maximized while negative sequence voltages have to be minimized. Current saturation restrictions imposed by the VSC characteristics have to be respected. This applies to each phase of each converter. Therefore, the generic optimization problem becomes
\begin{equation}
\begin{aligned}
