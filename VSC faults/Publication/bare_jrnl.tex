...
Given a basic system as the one depicted in Figure \ref{fig:sys_p}, the goal is to optimize the voltages at the point of common coupling. The fault is symbolized by a fault impedance, which in reality can either represent a balanced or unbalanced fault.

\begin{figure}[!htb]\scriptsize \centering
\begin{circuitikz}[european]
\thicklines

\draw (-3,0) to [sV, v_=$\underline{V}_{t}$] (-3,2);
\draw (-5,2) to [R, l=$\underline{Z}_{t}$] (-3,2);
\draw (-3.25,0) to [short] (-2.75,0);
\draw (-5.25,0) to [short] (-4.75,0);
\node at (-7,1.3) {PCC};
\node at (-9.5,1.3) {VSC};
\node at (-7,2.7) {$\underline{V}_p$};
\node at (-5,2.7) {$\underline{V}_f$};
\draw (-5,2) to [short] (-5.2,1);
\draw (-5.2,1) to [short] (-4.8,1);
\draw[-{Latex[length=3mm]}] (-4.8,1) to [short] (-5,0);
\draw (-7,2) to [R, l=$\underline{Z}_v$] (-5,2);
\draw[line width=0.65mm] (-7,2.5) to [short] (-7,1.5);
\draw[line width=0.65mm] (-5,2.5) to [short] (-5,1.5);
\draw (-9,2) to [R, l=$\underline{Z}_z$, i=$\underline{I}$] (-7,2);
\draw (-10.0,2) to [sdcac] (-9.0,2);


\end{circuitikz}
\caption{Single-phase representation of the basic system under a fault}
\label{fig:sys_p}
\end{figure}
The VSC is typically modeled as three-phase voltage sources in series with the filters impedance of the form $\underline{Z}_z$. Its control strategy directly adjusts the voltages to so that currents can indirectly meet the references \cite{andani2018controller}. However, the approach followed here consists of treating it as a current source that injects a current $\underline{I}$ into the system. This is precisely the variable to control in order to improve the voltage $\underline{V}_p$. Such current it constrained to not provide any zero sequence component and simultaneously respecting the limits. Therefore, the optimization problem reads as
\begin{equation}
\begin{aligned}
...
\label{eq:opt1}
\end{equation}
where the positive and negative sequence components of voltage $\un{V}_p$ are symbolically expressed as functions of the currents, and $I_{\text{max}}$ is the maximum allowed current by the converter. Considering that faults are strong enough so that the converter is incapable of raising the voltage $|\un{V}^{+}_p(\un{I})|$ to values higher than 1, the optimization problem can be rewritten:
\begin{equation}
\begin{aligned}
\min_{\un{I}} \quad & \lambda^+(1 - |\un{V}^+_p(\un{I}) |) + \lambda^-(0 - |\un{V}^-_p(\un{I})|) \ , \\
\textrm{s.t.} \quad & \text{max}(\un{I}^{a}, \un{I}^{b}, \un{I}^{c}) \leq I_{\text{max}} \ ,\\
...
\label{eq:opt2}
\end{equation}
Removing the outer absolute values has been proved to be beneficial in terms of computational time. The voltages are related to the currents by means of Kirchoff's laws. To compact the problem, the Thevenin grid equivalent has been transformed to a Norton equivalent with currents $\un{I}_{t}=\un{V}_{t}/\un{Z}_{t}$. Therefore:
\begin{equation}
  \begin{pmatrix}
    \un{I}^{a}\\
    \un{I}^{b}\\
  ...
  \end{pmatrix} \ , 
\end{equation}
where for a system as the one depicted in Figure \ref{fig:sys_p} the admittance matrix $\mathbf{Y}$ is a $6\times 6$ matrix which can be flexibly constructed depending on the grid impedances $\un{Z}_{v}$, $\un{Z}_f$ and $\un{Z}_t$. Finally, the sequence components of the voltages are obtained by means of Fortescue's transformation \cite{fortescue1918method}:
\begin{equation}
  \begin{pmatrix}
    \un{V}^{0}_{p} \\
    \un{V}^{+}_{p} \\
...
Although the study can also be performed in the sequence components reference frame as in \cite{taul2020modeling}, each type of fault has to be studied individually. Consequently, even if both approaches have been proved to be generate the same results, this paper opts for analyzing the faults in the natural reference frame for greater simplicity. 
...
\begin{figure}[!htb]\centering \footnotesize
    \begin{subfigure}[!htb]{.2\textwidth}
      \centering
          \begin{tikzpicture}[trim axis right,trim axis left]
              \pgfplotsset{width=2cm, height=2.5cm}
