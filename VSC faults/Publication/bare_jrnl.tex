...
Given a generic system as the one depicted in Figure \ref{fig:sys_p}, the goal is to optimize the voltages at the point of common coupling. The fault is symbolized by a fault impedance, which in reality can either represent a balanced or unbalanced fault.
...
\end{circuitikz}
\caption{Single-phase representation of the generic system under fault conditions}
\label{fig:sys_p}
\end{figure}
...
\node at (-7,1.3) {PCC1};
\node at (-9.5,2.6) {VSC1};
\node at (-7,2.7) {$\underline{V}_{p1}$};
\draw (-7,2) to [R, l=$\underline{Z}_{v1}$, european] (-5.5,2);
\draw[line width=0.65mm] (-7,2.5) to [short] (-7,1.5);
% \draw[line width=0.65mm] (-3,2.5) to [short] (-3,1.5);
% \node at (-3,2.7) {$\underline{V}_{f}$};
\draw (-9,2) to [R, l=$\underline{Z}_{x1}$, i=$\underline{I}_1$, european] (-7.0,2);
\draw (-10.0,2) to [sdcac] (-9.0,2);

\draw (-10.0,-0) to [sdcac] (-9.0,-0);
\draw (-9,-0) to [R, l=$\underline{Z}_{x2}$, i=$\underline{I}_2$, european] (-7,-0);
\draw (-7,-0) to [R, l=$\underline{Z}_{v2}$, european] (-5.5,-0);
\draw[line width=0.65mm] (-7,0.5) to [short] (-7,-0.5);
\node at (-7,-0.7) {PCC2};
\node at (-9.5,0.6) {VSC2};
\node at (-7,0.7) {$\underline{V}_{p2}$};

\draw (-10.0,-2) to [sdcac] (-9.0,-2);
\draw (-9,-2) to [R, l=$\underline{Z}_{xn}$, i=$\underline{I}_n$, european] (-7,-2);
\draw (-7,-2) to [R, l=$\underline{Z}_{vn}$, european] (-5.5,-2);
\draw[line width=0.65mm] (-7,-1.5) to [short] (-7,-2.5);
\node at (-7,-2.7) {PCCn};
\node at (-9.5,-1.4) {VSCn};
\node at (-7,-1.3) {$\underline{V}_{pn}$};
...
\draw[line width=0.4mm, dotted] (-6.25,-0.3) to [short] (-6.25,-1.4);
...
VSCs are typically modeled as three-phase voltage sources in series with the filters impedance of the form $\underline{Z}_z$. The control strategy directly adjusts the voltages to so that currents can indirectly meet the references \cite{andani2018controller}. However, the approach followed here consists of treating it as a current source that injects a current $\underline{I}$ into the system. This is precisely the variable to control in order to improve the voltage $\underline{V}_p$.





Such current it constrained to not provide any zero sequence component and simultaneously respecting the limits. Therefore, the optimization problem reads as
\begin{equation}
\begin{aligned}
