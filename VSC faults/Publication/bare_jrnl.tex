...
\begin{cases} 
   ...
   \label{eq:gc2}
\end{equation}
where the values of the constants are gathered in Table \ref{tab:param_gc}. Constants $k_p$ and $k_n$, which represent the slope of the droop, are set as fixed quantities. Because of this, the application of the grid code rules implies that maximum phase currents are hardly ever equal to the maximum allowed by the VSC, even when strong faults occur. Thus, an adaptative control can be formulated so that currents tend to the maximum permitted value while the droop characteristic is preserved. This strategy is denoted by ADA, as in essence it consists of an adaptative mechanism. In order to fairly compare between methodologies, $k_p$ and $k_n$ are always equal. The expressions for the ADA strategy are reduced to
\begin{equation}
  \begin{cases}
    |\underline{I}_k^+| = k_p(V^+_{\text{high}} - |\underline{V}^+_{pk}|) \\
    |\underline{I}_k^-| = k_n(|\underline{V}^-_{pk}| - V^-_{\text{low}})
  \end{cases}
\end{equation}
that is, the ADA control always mimics a droop characteristic, although this is not necessarily linear as $k_p$ and $k_n$ vary.

\subsection{Summary of support techniques}
The paper evaluates and compares three support strategies, which are:
\begin{itemize}
  \item OPT: corresponds to the solution of the optimization problem in \eqref{eq:opt2}, particularized for each system. It provides, by definition, the best solution.
  \item GC: is the application of the grid code rules shown in \eqref{eq:gc1} and \eqref{eq:gc2}, with $k_p=k_n=1.25$. It should generate the less convenient solution out of the three analyzed strategies. 
  \item ADA: resembles the grid code rules from \eqref{eq:gc1} and \eqref{eq:gc2} with an adapted value of the constants $k_p=k_n$ so that the VSC operates close to the current limitations. 
\end{itemize}
...
\label{eq:opt_1c}
\end{equation}
Because of the nature of the converter, it is also imposed that the sum of the three phase currents becomes null. The procedure to solve the optimization problem is first responsible for initializing the admittances matrices $\mathbf{\un{Y}_v}$ as in \eqref{eq:Yvmat}, and then, constructing $\mathbf{\un{Y}_f}$. Next, currents are initialized to a random array of values, for instance. The Mystic package is subsequently called to solve the optimization problem stated in \eqref{eq:opt_1c} up to a given relative precision $\delta$, being $\delta=1e-6$ for example. Positive and negative sequence voltages are evaluated to determine the optimality of the solution by means of \eqref{eq:fortescue}. In case of sweeping a range of $n$ scenarios, this process is repeated $n$ times. Currents are eventually also transformed to positive and negative sequence values, since the final goal is to evaluate their values in this frame of reference. Unless noted otherwise, the corresponding baseline parameters of the system in Figure \ref{fig:sys_p1} are indicated in Table \ref{tab:param}. 
...
\begin{table}\centering
  ...
  \label{tab:param}
\end{table}
Solving the problem with the aforementioned steps for a balanced fault yields the results shown in Figure \ref{fig:Zf_1conv_3x}, where impedance $\un{Z}_x$ denotes the fault impedance connected to each phase. The results suggest that the OPT case is the preferred one, as its associated objective function is always the smallest, as expected. This optimal solution is achieved by distributing the currents between the real and imaginary parts. The current capabilities are roughly equally attributed among both. The ADA solution and the GC one present only slight differences, so varying the slope of the grid code current definitions does not yield remarkable benefits. However, this consitutes a particular scenario since no negative sequence current ought to be injected. The only difference between ADA and GC is that the former specifies a moderately higher current.
      ...
      \centering
          \begin{tikzpicture}[trim axis right,trim axis left]
              ...
              \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={${I}^+_{re}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.01}, axis line style = thick, ytick distance={0.2}, xmin=0.03, xmax=0.07, scaled y ticks=false, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}]
                  ...
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_3x_Ip1re.csv};
        ...
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                ...
                \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={${I}^+_{im}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.01}, axis line style = thick, xmin=0.03, xmax=0.07, ytick distance = {0.1}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}]
                  ...
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_3x_Ip1im.csv};
        ...
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                ...
                \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={${I}^-_{re}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.01}, axis line style = thick, xmin=0.03, xmax=0.07, ytick distance = {0.05}, ymax = 0.1, ymin=-0.1, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}]
                  ...
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_3x_In1re.csv};
      ...
      \begin{subfigure}[!htb]{.12\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  ...
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_3x_In1im.csv};
        ...
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                ...
                \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={$|V^+|$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.01}, axis line style = thick, xmin=0.03, xmax=0.07, ytick distance = {0.1}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}]
                  ...
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_3x_Vp1.csv};
      ...
      \begin{subfigure}[!htb]{.12\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  ...
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_3x_Vn1.csv};
...
\begin{subfigure}[!htb]{0.4\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  ...
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_3x_f_obj.csv};
                  \addplot[very thick, green, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/gc_3x_f_obj.csv};
                  \legend{OPT, GC, ADA};  % sta: static = GC, gc: ADA
                  \end{axis}
              \end{tikzpicture}
        \end{subfigure}
    \end{center}
  ...
  \end{figure}
Significantly different results are obtained in the case of a line to line fault, as shown in Figure \ref{fig:Zf_1conv_LL}. This unbalanced fault causes the optimal currents to be non-null for the imaginary part of the negative sequence. While the OPT solution specifies almost constant current values in $I^+_{re}$, $I^+_{im}$ and $I^-_{im}$, much more varying currents are injected in the case of ADA and GC. Thanks to the adaptative control, both $I^+_{im}$ and $I^-_{im}$ are larger than in the GC situation. Consequently, more voltage support is provided, which causes ADA to be a near-optimal solution despite not injecting any active power, specially for large values of $\un{Z}_x$. 
...
\begin{figure}[]\centering \tiny
    \begin{subfigure}[!htb]{0.12\textwidth}
      \centering
          \begin{tikzpicture}[trim axis right,trim axis left]
              \pgfplotsset{width=4.5cm, height=3.5cm}
              \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={${I}^+_{re}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.01}, axis line style = thick, ytick distance={0.1}, xmin=0.12, xmax=0.17, scaled y ticks=false, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/opt_LL_Ip1re.csv};
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_LL_Ip1re.csv};
    ...
    \begin{subfigure}[!htb]{0.12\textwidth}
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                \pgfplotsset{width=4.5cm, height=3.5cm}
                \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={${I}^+_{im}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.01}, axis line style = thick, xmin=0.12, xmax=0.17, ytick distance = {0.05}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}, scaled y ticks=false,  scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/opt_LL_Ip1im.csv};
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_LL_Ip1im.csv};
      ...
      \begin{subfigure}[!htb]{.12\textwidth}
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                \pgfplotsset{width=4.5cm, height=3.5cm}
                \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={${I}^-_{re}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.01}, axis line style = thick, xmin=0.12, xmax=0.17, ytick distance = {0.04}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3},  scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/opt_LL_In1re.csv};
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_LL_In1re.csv};
      ...
      \begin{subfigure}[!htb]{.12\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  \pgfplotsset{width=4.5cm, height=3.5cm}
                  \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={${I}^-_{im}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.01}, axis line style = thick,  xmin=0.12, xmax=0.17, ytick distance = {0.1}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3},   scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/opt_LL_In1im.csv};
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_LL_In1im.csv};
        ...
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                ...
                \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={$|V^+|$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.01}, axis line style = thick, xmin=0.12, xmax=0.17, ytick distance = {0.02}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/opt_LL_Vp1.csv};
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_LL_Vp1.csv};
      ...
      \begin{subfigure}[!htb]{.12\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  ...
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/opt_LL_Vn1.csv};
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_LL_Vn1.csv};
...
\begin{subfigure}[!htb]{0.4\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  \pgfplotsset{width=9.4cm, height=3.51cm}
                  \begin{axis}[grid=major, xlabel={$\un{Z}_{x}$}, ylabel={$f$}, /pgf/number format/.cd, legend style={at={(0.98,0.5)},anchor=south east,legend columns=1, xmin=0.12, xmax=0.17, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.005}, axis line style = thick, ytick distance = {0.04},  yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/opt_LL_f_obj.csv};
                  \addplot[very thick, red] table[x=x, y=y, col sep=comma] {Data/Results_1conv_Zf_v1/sta_LL_f_obj.csv};
  ...
  \end{figure}
In essence, ADA does not retain the same linear droop characteristics than the GC rules defined in \eqref{eq:gc1} and \eqref{eq:gc2}. Figure \ref{fig:Zf_1conv_LL2} shows the coupling between the constants $k_p=k_n$ and the positive and negatives sequence voltages for a line to line fault with $\un{Z}_x \in [0.005,0.17]$. The evolutions are quadratic, which suggests that a linear dependence between injected sequence currents and voltages would not result in near-optimal operating points. % talk about the symmetry between voltages, and that it happens for all unbalanced faults?? k = factor * V+ ^ 1.5


\begin{figure}[]\centering \tiny
      \centering
          \begin{tikzpicture}[trim axis right,trim axis left]
              \pgfplotsset{width=8cm, height=4.0cm}
              \begin{axis}[grid=major, ylabel={$|V|$}, xlabel={$k_p=k_n$}, /pgf/number format/.cd, legend style={at={(0.98,0.3)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.2}, axis line style = thick, ytick distance={0.1}, xmin=1.4, xmax=3.2, scaled y ticks=false, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black] table[x=x, y=y, col sep=semicolon] {Data/Results_1conv_Zf_v1/kV1_LL.csv};
                  \addplot[very thick, gray] table[x=x, y=y, col sep=semicolon] {Data/Results_1conv_Zf_v1/kV2_LL.csv};
              \legend{$|\un{V}^+|$, $|\un{V}^-|$};
              \end{axis}
          \end{tikzpicture}
    \caption{Dependence between constants $k_p=k_n$ and the sequence voltages in the ADA approach for a line to line fault}
    \label{fig:Zf_1conv_LL2}
  \end{figure}


% plot i1 vs v1 and i2 vs v2

% show a plot of the value that the k takes for ADA for the previous LL fault. 

% for this reason, we propose a quadratic profile... Tune it heuristically. And test it.



% maybe I can study the same but with larger Zx so it does not operate saturated
...
The $R_1/X_1$ variation analysis performs a swept for a range of a varying angle of the $\un{Z}_{v1}$ impedance, while the absolute value is preserved. The imaginary part of the currents (reactive component) tends to be superior than the real part (active component) for a small $R_1/X_1$ ratio. The currents evolve asymptotically, in the sense that even if the resistive part of the impedance is substantially larger than the reactance, the same prioritization applies. The ADA strategy dedicates most of its current capability to inject negative sequence reactive current, whereas the optimal solution tends to balance the currents not only between sequences but also between real and imaginary parts. Although ADA injects larger currents (by definition the saturation is forced), it does not provide a more favorable solution that the classical grid code GC. Differences are small, yet operating the VSC at its maximum current capabilities has a detrimental effect on the negative sequence voltage. 
      ...
      \centering
          \begin{tikzpicture}[trim axis right,trim axis left]
              ...
              \begin{axis}[grid=major, xlabel={$R_1/X_1$}, ylabel={${I}^+_{re}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={1}, axis line style = thick, ytick distance={0.05}, xmin=0.5, xmax=5, scaled y ticks=false, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}]
                  ...
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/gc_LG_Ip1re.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/sta_LG_Ip1re.csv};
              % \legend{BF, OPT};
              \end{axis}
          \end{tikzpicture}
    \end{subfigure}
    ...
    \begin{subfigure}[!htb]{0.12\textwidth}
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                ...
                \begin{axis}[grid=major, xlabel={$R_1/X_1$}, ylabel={${I}^+_{im}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={1}, axis line style = thick, xmin=0.5, xmax=5, ytick distance = {0.1}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  ...
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/gc_LG_Ip1im.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/sta_LG_Ip1im.csv};
                % \legend{OPT};
                \end{axis}
            \end{tikzpicture}
      \end{subfigure}
        ...
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                ...
                \begin{axis}[grid=major, xlabel={$R_1/X_1$}, ylabel={${I}^-_{re}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={1}, axis line style = thick, xmin=0.5, xmax=5, ytick distance = {0.1}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3},]
                  ...
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/gc_LG_In1re.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/sta_LG_In1re.csv};
                % \legend{BF, OPT};
                \end{axis}
            \end{tikzpicture}
      \end{subfigure}
      ...
      \begin{subfigure}[!htb]{.12\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  ...
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/gc_LG_In1im.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/sta_LG_In1im.csv};
                  \end{axis}
              \end{tikzpicture}
        \end{subfigure}
      \begin{subfigure}[!htb]{.12\textwidth}
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                \pgfplotsset{width=4.5cm, height=3.5cm}
                \begin{axis}[grid=major, xlabel={$R_1/X_1$}, ylabel={$|V^+|$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={1}, axis line style = thick, xmin=0.5, xmax=5, ytick distance = {0.02}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3},]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/opt_LG_Vp1.csv};
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/gc_LG_Vp1.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/sta_LG_Vp1.csv};
                % \legend{BF, OPT};
                \end{axis}
            \end{tikzpicture}
      \end{subfigure}
      ...
      \begin{subfigure}[!htb]{.12\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  \pgfplotsset{width=4.5cm, height=3.5cm}
                  \begin{axis}[grid=major, xlabel={$R_1/X_1$}, ylabel={$|V^-|$}, /pgf/number format/.cd, legend style={at={(0.98,0.23)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={1}, axis line style = thick,  xmin=0.5, xmax=5, ytick distance = {0.04}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/opt_LG_Vn1.csv};
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/gc_LG_Vn1.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/sta_LG_Vn1.csv};
                  \end{axis}
              \end{tikzpicture}
        \end{subfigure}
        \vspace{-0.2cm}
...
\begin{subfigure}[!htb]{0.4\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  \pgfplotsset{width=9.4cm, height=3.51cm}
                  \begin{axis}[grid=major, xlabel={$R_1/X_1$}, ylabel={$f$}, /pgf/number format/.cd, legend style={at={(0.98,0.17)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.5}, xmin=0.5, xmax=5, axis line style = thick, ytick distance = {0.05},  yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3},]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/opt_LG_f_obj.csv};
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/gc_LG_f_obj.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_RX_v1/sta_LG_f_obj.csv};
                  \legend{OPT, ADA, GC};
                  \end{axis}
              \end{tikzpicture}
        \end{subfigure}
    \end{center}
    \caption{Influence of the currents on the objective function for the line to ground fault with a varying $R_1/X_1$ ratio and $\un{Z}_{ag}=0.1$}
    \label{fig:RX_1conv_LG}
  \end{figure}
Noticeably, the objective function is further minimized in the OPT case. In percentual terms there is an approximate 40\% improvement with respect to the GC approach. This is achieved balancing the currents. When the VSC operates at the optimal conditions, it takes full advantage of the current capabilities, just like it happened in the line to line fault shown in Figure \ref{fig:Zf_1conv_LL}. Thus, a line to ground fault with a predominant resistive characteristic is much more prone to being improved by means of adequately distributing the currents across sequences and real/imaginary parts. Across all range of $R_1/X_1$ values analyzed in Figure \ref{fig:RX_1conv_LG}, the injected active power remains between 0.405 and 0.401 whereas the reactive power is around 0.486 to 0.461. In short, the injection of active power has a positive contribution. 


\subsection{Cable length variation analysis}
Figure \ref{fig:sys_c2} represents a third case regarding single converter systems, which deals with a hypothetical submarine cable modeled with its $\pi$ equivalent. The data selected to model the cable are extracted from \cite{cheah2017offshore} and adapted to per unit values. Table \ref{tab:cable} presents them. The followed approach to analyze the system is identical to the previous ones, except for the reduction of the set of elements including the cable and the grid. The corresponding Thevenin equivalent is defined by
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
Similarly as before, a Norton equivalent is preferred. The current $\un{I}'_t$ is simply equal to $\un{V}'_t/\un{Z}'_t$, which yields a basic system as the one in Figure \ref{fig:sys_p1}. 

The analysis performed considers a cable with a varying length, up to 75 km. Its impact on the Thevenin equivalent parameters is shown in Figure \ref{fig:dist_th}. The voltage experiences a subtle increase, while the imaginary part of the equivalent impedance grows considerably with distance, yet the $\Re({\un{Z}'_t}) / \Im({\un{Z}'_t})$ ratio tends to rise. 
...
The cases OPT, ADA and GC are again evalauted, as depicted in Figure \ref{fig:cable_1conv_LL2}. The OPT situation unveils that, contrary to ADA and GC, imaginary currents ought to be rather stable. While in practical terms its positive sequence voltage is the same as the one from the standard grid code (GC), the negative sequence voltage becomes about 10\% lower. Therefore it can be stated that the optimization yields the most favorable value in general terms, although as it is the case here, some sequence may be worse off than the other options. Compared to GC, ADA maximizes the positive sequence voltage mainly by injecting larger negative sequence imaginary currents. However, an improvement of about 2\% in the objective function may not justify operating the converter at its maximum current capabilities.

Increasing the cable distance causes the negative sequence voltage to grow, while the positive sequence voltage also tends to grow. This phenomena differs from the profiles obtained in Figures \ref{fig:Zf_1conv_LL} and \ref{fig:RX_1conv_LG}, where both voltages either simultaneously approached the objective value (1 and 0 respectively), or distanced from it.
...
\begin{figure}[]\centering \tiny
    \begin{subfigure}[!htb]{0.12\textwidth}
      \centering
          \begin{tikzpicture}[trim axis right,trim axis left]
              \pgfplotsset{width=4.5cm, height=3.5cm}
              \begin{axis}[grid=major, xlabel={$R_1/X_1$}, ylabel={${I}^+_{re}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={15}, axis line style = thick, ytick distance={0.1}, xmin=1, xmax=75, scaled y ticks=false, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/opt_LL_Ip1re.csv};
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/gc_LL_Ip1re.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/sta_LL_Ip1re.csv};
              % \legend{BF, OPT};
              \end{axis}
          \end{tikzpicture}
    \end{subfigure}
    ...
    \begin{subfigure}[!htb]{0.12\textwidth}
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                \pgfplotsset{width=4.5cm, height=3.5cm}
                \begin{axis}[grid=major, xlabel={$R_1/X_1$}, ylabel={${I}^+_{im}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={15}, axis line style = thick, xmin=1, xmax=75, ytick distance = {0.05}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/opt_LL_Ip1im.csv};
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/gc_LL_Ip1im.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/sta_LL_Ip1im.csv};
                % \legend{OPT};
                \end{axis}
            \end{tikzpicture}
      \end{subfigure}
        ...
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                ...
                \begin{axis}[grid=major, xlabel={$R_1/X_1$}, ylabel={${I}^-_{re}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={15}, axis line style = thick, xmin=1, xmax=75, ytick distance = {0.05}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3},]
                  ...
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/gc_LL_In1re.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/sta_LL_In1re.csv};
                % \legend{BF, OPT};
                \end{axis}
            \end{tikzpicture}
      \end{subfigure}
      ...
      \begin{subfigure}[!htb]{.12\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  \pgfplotsset{width=4.5cm, height=3.5cm}
                  \begin{axis}[grid=major, xlabel={Distance (km)}, ylabel={${I}^-_{im}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={15}, axis line style = thick,  xmin=1, xmax=75, ytick distance = {0.1}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3},]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/opt_LL_In1im.csv};
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/gc_LL_In1im.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/sta_LL_In1im.csv};
                  \end{axis}
              \end{tikzpicture}
        \end{subfigure}
      \begin{subfigure}[!htb]{.12\textwidth}
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                ...
                \begin{axis}[grid=major, xlabel={Distance (km)}, ylabel={$|V^+|$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={15}, axis line style = thick, xmin=1, xmax=75, ytick distance = {0.02}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3},]
                  ...
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/gc_LL_Vp1.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/sta_LL_Vp1.csv};
                % \legend{BF, OPT};
                \end{axis}
            \end{tikzpicture}
      \end{subfigure}
      ...
      \begin{subfigure}[!htb]{.12\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  \pgfplotsset{width=4.5cm, height=3.5cm}
                  \begin{axis}[grid=major, xlabel={Distance (km)}, ylabel={$|V^-|$}, /pgf/number format/.cd, legend style={at={(0.98,0.23)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={15}, axis line style = thick,  xmin=1, xmax=75, ytick distance = {0.04}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/opt_LL_Vn1.csv};
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/gc_LL_Vn1.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/sta_LL_Vn1.csv};
                  \end{axis}
              \end{tikzpicture}
        \end{subfigure}
        \vspace{-0.2cm}
...
\begin{subfigure}[!htb]{0.4\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  \pgfplotsset{width=9.4cm, height=3.51cm}
                  \begin{axis}[grid=major, xlabel={Distance (km)}, ylabel={$f$}, /pgf/number format/.cd, legend style={at={(0.98,0.17)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={5}, xmin=1, xmax=75, axis line style = thick, ytick distance = {0.02},  yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3},]
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/opt_LL_f_obj.csv};
                  \addplot[very thick, green] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/gc_LL_f_obj.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_1conv_cable_v1/sta_LL_f_obj.csv};
                  \legend{OPT, ADA, GC};
                  \end{axis}
              \end{tikzpicture}
        \end{subfigure}
    \end{center}
    \caption{Influence of the currents on the objective function for a line to line fault with a varying cable distance}
...
A two-converter case is proposed in order to spot the interaction between both and compare the performance of the optimization with the grid code rules. Figure \ref{fig:sys_p2} shows a single-phase representation of the system under study. The converters are coupled to the grid by means of slightly different impedances $\un{Z}_{v1}$ and $\un{Z}_{v2}$. Table \ref{tab:param2} shows their values together with other input data. 
...
\begin{figure}[!htb]\centering \tiny
    \begin{subfigure}[!htb]{0.12\textwidth}
      \centering
          \begin{tikzpicture}[trim axis right,trim axis left]
              \pgfplotsset{width=4.5cm, height=3.5cm}
              \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={${I}^+_{re}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.02}, axis line style = thick, ytick distance={0.1}, xmin=0.004, xmax=0.1, scaled y ticks=false, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}]
                  \addplot[very thick, black] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/opt_3x_Ip1re.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/opt_3x_Ip2re.csv};
              % \legend{BF, OPT};
              \end{axis}
          \end{tikzpicture}
    \end{subfigure}
    ...
    \begin{subfigure}[!htb]{0.12\textwidth}
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                \pgfplotsset{width=4.5cm, height=3.5cm}
                \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={${I}^+_{im}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.02}, axis line style = thick, xmin=0.004, xmax=0.1, ytick distance = {0.1}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}]
                  \addplot[very thick, black] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/opt_3x_Ip1im.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/opt_3x_Ip2im.csv};
                % \legend{OPT};
                \end{axis}
            \end{tikzpicture}
      \end{subfigure}
      ...
      \begin{subfigure}[!htb]{.12\textwidth}
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                \pgfplotsset{width=4.5cm, height=3.5cm}
                \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={${I}^-_{re}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.02}, axis line style = thick, xmin=0.004, xmax=0.1, ytick distance = {0.05}, ymax = 0.1, ymin=-0.1, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}]
                  \addplot[very thick, black] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/opt_3x_In1re.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/opt_3x_In2re.csv};
                % \legend{BF, OPT};
                \end{axis}
            \end{tikzpicture}
      \end{subfigure}
      ...
      \begin{subfigure}[!htb]{.12\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  \pgfplotsset{width=4.5cm, height=3.5cm}
                  \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={${I}^-_{im}$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.02}, axis line style = thick,  xmin=0.004, xmax=0.1, ytick distance = {0.05}, ymax = 0.1, ymin = -0.1,  yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}]
                  \addplot[very thick, black] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/opt_3x_In1im.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/opt_3x_In2im.csv};
                  \end{axis}
              \end{tikzpicture}
        \end{subfigure}
      ...
      \begin{subfigure}[!htb]{.12\textwidth}
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                \pgfplotsset{width=4.5cm, height=3.5cm}
                \begin{axis}[grid=major, xlabel={$\un{Z}_x$}, ylabel={$|V^+|$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.02}, axis line style = thick, xmin=0.004, xmax=0.1, ytick distance = {0.1}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}, ymax=0.5]
                  \addplot[very thick, black] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/opt_3x_Vp1.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/opt_3x_Vp2.csv};
                % \legend{BF, OPT};
                \end{axis}
            \end{tikzpicture}
      \end{subfigure}
      ...
      \begin{subfigure}[!htb]{.12\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  ...
                  \addplot[very thick, black] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/opt_3x_Vn1.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/opt_3x_Vn2.csv};
                  \legend{VSC1, VSC2};
                  \end{axis}
              \end{tikzpicture}
        \end{subfigure}
...
\begin{subfigure}[!htb]{0.4\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  \pgfplotsset{width=9.4cm, height=3.51cm}
                  \begin{axis}[grid=major, xlabel={$\un{Z}_{x}$}, ylabel={$f$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.01}, xmin=0.004, xmax=0.1, axis line style = thick, ytick distance = {0.2},  yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}, ymin=1.1]
                  \addplot[very thick, black] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/opt_3x_f_obj.csv};
                  \end{axis}
              \end{tikzpicture}
        \end{subfigure}
    \end{center}
