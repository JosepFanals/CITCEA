...
Positive sequence voltages have to be maximized while negative sequence voltages have to be minimized. The zero sequence component of the voltages is a magnitude likely to become non-null under asymmetrical faults. Nevertheless, as VSCs are unable to inject zero sequence currents, it will remain an uncontrolled variable in the sense that no efforts will be made towards reducing it. Current saturation restrictions imposed by the VSC characteristics have to be respected. This applies to each phase of each converter. Therefore, the generic optimization problem is
\begin{equation}
\begin{aligned}
...
\begin{equation}
  ...
  \label{eq:Ifort1}
\end{equation}
where $\un{I}^0$ is null as the VSC is incapable of injecting it. The $k$ index related to identifying a given VSC has been omitted to alleviate the notation. Explicitly, \eqref{eq:Ifort1} becomes
\begin{equation}
  \begin{cases}
    \un{I}_a = \un{I}^+ + \un{I}^- \\
