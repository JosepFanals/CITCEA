...
\begin{cases} 
   ...
   \label{eq:gc2}
\end{equation}
Constants $k_p$ and $k_n$, which represent the slope of the droop, are set as fixed quantities. The converter would therefore operate at saturated conditions during extreme faults because either the positive sequence voltage would reach the lower threshold $V^+_{\text{low}}$ or the negative sequence voltage would surpass the upper threshold $V^-_{\text{high}}$. However, in severe faults, it is still possible that both limits are surpassed, or at least, approached. In case \eqref{eq:gc1} and \eqref{eq:gc2} were applied simultaneously, the required current to be injected by the VSC could potentially exceed the $I_{\text{max},k}$ limitation. Thus, it makes sense to establish a prioritization. 

This paper considers the two straightforward prioritizations for voltage support operation of converters following grid codes. One strategy is to have the positive sequence current following \eqref{eq:gc1} and injecting the remaining maximum allowed current in the negative sequence. This methodology will be commonly referred to as GCP. The other option, abbreviated as GCN, is concerned with obeying \eqref{eq:gc2} and analogously injecting the maximum positive sequence current that respects the limits. During all situations, the converters work under saturated conditions (current limits reached) as the non-prioritized current is set to the allowed maximum. Also, only reactive power is injected. The procedure to determine the non-prioritized current is depicted below.
...
\begin{equation}
  ...
  \label{eq:Iphaseb}
\end{equation}
The goal is to determine the maximum current that respects the limitations. To do so, all three phase currents are set to the maximum allowed current. It is know beforehand that not all three phases will operate at saturation. Yet, one can imagine that if currents are gradually increased, one phase will reach saturation before the others. This will act as the most restrictive option. Consequently, the phase currents in \eqref{eq:Iphaseb} are squared in order to express them as functions of $I^+_{re}$, $I^+_{im}$, $I^-_{re}$ and $I^-_{im}$:
...
A procedure to find these four components $I^+_{re}$, $I^+_{im}$, $I^-_{re}$ and $I^-_{im}$ from \eqref{eq:Ifin} is described step-by-step. 

When the positive sequence is prioritized (GCP):
\begin{enumerate}

  \item Angles $\alpha^+$ and $\alpha^-$ are computed by knowing the phasors $\un{V}^+$, $\un{V}^-$, and shifting them $\pm \frac{\pi}{2}$ as only reactive power has to be injected:
  \begin{equation}
    \begin{cases}
    ...
    \end{cases}
  \end{equation}
  where $\angle$ denotes the function that extracts the angle of a complex magnitude.
  \item The absolute value of the positive sequence current is found with the grid code rules in \eqref{eq:gc1}. Its angle is equal to $\alpha^+$. Thus, $\un{I}^+$ is fully known, and this implies that $I^+_{re}$ and $I^+_{im}$ also are. 
  \item There are two remaining unknowns: $I^-_{re}$ and $I^-_{im}$. One component can be expressed as a function of the other. For instance:
  \begin{equation}
    I^-_{im} = \tan(\alpha^-) I^-_{re} \ .
    \label{eq:alphn}
  \end{equation}
  \item At this point, only $I^-_{re}$ acts as an unkown. The three equations in \eqref{eq:Ifin} are solved in order to find $I^-_{re}$. Consequently, there are 6 solutions (two for each of the three quadratic equations). The solutions can be grouped in an array, expressed as $[I^{a1-}_{re}, I^{a2-}_{re}, I^{b1-}_{re}, I^{b2-}_{re}, I^{c1-}_{re}, I^{c2-}_{re}]$. 
  \item Employing \eqref{eq:alphn} and the six $I^-_{re}$ elements, the corresponding array of $I^-_{im}$ elements is found. Generically, this is $[I^{a1-}_{im}, I^{a2-}_{im}, I^{b1-}_{im}, I^{b2-}_{im}, I^{c1-}_{im}, I^{c2-}_{im}]$.
  \item Out of the six pairs of $I^-_{re}$ and $I^-_{im}$ currents, three of them have to be discarded. The reason is that although they obey the tangent relation in \eqref{eq:alphn}, they are placed in the wrong quadrant. This yields three remaining pairs of currents $[I^{a-}_{re}, I^{b-}_{re}, I^{c-}_{re}]$ and $[I^{a-}_{im}, I^{b-}_{im}, I^{c-}_{im}]$, that is, there is only one valid solution for each of the three equations in \eqref{eq:Ifin}. 
  \item The currents are compacted as:
  \begin{equation}
    \begin{cases}
    \un{I}^{a-} = I^{a-}_{re} + jI^{a-}_{im} \\
    \un{I}^{b-} = I^{b-}_{re} + jI^{b-}_{im} \\
    \un{I}^{c-} = I^{c-}_{re} + jI^{c-}_{im} \\
    \end{cases}
  \end{equation}
  \item The current with the smallest absolute value is the one to pick, because this way only one phase will be at saturated conditions. Thus, the final negative sequence current becomes:
  \begin{equation}
    I^- = \min\left(I^{a-}, I^{b-}, I^{c-}\right) \ .
  \end{equation}
\end{enumerate}
...
Similarly, when the prioritization is applied to the negative sequence (GCN):
...
\item Angles $\alpha^+$ and $\alpha^-$ are computed by knowing the phasors $\un{V}^+$, $\un{V}^-$, and shifting them $\pm \frac{\pi}{2}$ as only reactive power has to be injected:
  \begin{equation}
    ...
    \end{cases}
  \end{equation}
  \item The absolute value of the negative sequence current is computed from \eqref{eq:gc2}, with its angle being equal to $\alpha^-$. Thus, the phasor $\un{I}^-$ is known, and also $I^-_{re}$ and $I^-_{im}$.
  \item The two remaining unknowns are $I^+_{re}$ and $I^+_{im}$. One component can be expressed as a function of the other:
  \begin{equation}
    I^+_{im} = \tan(\alpha^+) I^+_{re} \ .
    \label{eq:alphp}
  \end{equation}
  \item At this point, the only unkown is $I^+_{re}$. The three equations in \eqref{eq:Ifin} are solved for $I^+_{re}$. The solutions can be grouped in an array, expressed as $[I^{a1+}_{re}, I^{a2+}_{re}, I^{b1+}_{re}, I^{b2+}_{re}, I^{c1+}_{re}, I^{c2+}_{re}]$. 
  \item With \eqref{eq:alphp} and the six $I^+_{re}$ currents, the array of $I^+_{im}$ elements is found:  $[I^{a1+}_{im}, I^{a2+}_{im}, I^{b1+}_{im}, I^{b2+}_{im}, I^{c1+}_{im}, I^{c2+}_{im}]$.
  \item Three of the six pairs of $I^+_{re}$ and $I^+_{im}$ currents are not valid. The reason is that although they obey \eqref{eq:alphp}, they are in the wrong quadrant. This yields three remaining pairs of currents $[I^{a+}_{re}, I^{b+}_{re}, I^{c+}_{re}]$ and $[I^{a+}_{im}, I^{b+}_{im}, I^{c+}_{im}]$.
  \item The currents are compacted as:
  \begin{equation}
    \begin{cases}
    \un{I}^{a+} = I^{a+}_{re} + jI^{a+}_{im} \\
    ...
    \end{cases}
  \end{equation}
  \item The current with the smallest absolute value is the relevant one, as it ensures that no limitations would be exceeded. Thus, the final positive sequence current becomes:
  \begin{equation}
    I^+ = \min\left(I^{a+}, I^{b+}, I^{c+}\right) \ .
  \end{equation}
\end{enumerate}
...
\draw (-0.5,1.35) to [R, l=$\underline{Z}_{t}$, european] (1,1.35);
\draw (1,1.35) to [short] (1.25,1.35);
\draw (-0.5, 1.35) to [short] (-0.75, 1.35);
\draw (1.25,1.35) to [sV, l_=$\underline{V}_t$] (1.25,-0.5);
\draw (1,-0.5) to [short] (1.5,-0.5);
% \draw (-2,1.35) to [short] (-2.5,1.35);
...
\begin{figure}[]\centering \tiny
    \begin{subfigure}[!htb]{0.12\textwidth}
      \centering
          \begin{tikzpicture}[trim axis right,trim axis left]
              \pgfplotsset{width=4.5cm, height=3.5cm}
              \begin{axis}[grid=major, xlabel={${Z}_x$}, ylabel={${I}^+$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.005}, axis line style = thick, ytick distance={0.1}, xmin=0.01, xmax=0.035, scaled y ticks=false, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_opt_LG_Ip1.csv};
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_opt_LG_Ip2.csv};
                  \addplot[very thick, cyan] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcp_LG_Ip1.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcp_LG_Ip2.csv};
                  \addplot[very thick, violet] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcn_LG_Ip1.csv};
                  \addplot[very thick, green, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcn_LG_Ip2.csv};
              \end{axis}
          \end{tikzpicture}
    \end{subfigure}
    \hspace{2.35cm}
        ...
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                ...
                \begin{axis}[grid=major, xlabel={${Z}_x$}, ylabel={${I}^-$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={0.005}, axis line style = thick, xmin=0.01, xmax=0.035, ytick distance = {0.1}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_opt_LG_In1.csv};
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_opt_LG_In2.csv};
                  \addplot[very thick, cyan] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcp_LG_In1.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcp_LG_In2.csv};
                  \addplot[very thick, violet] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcn_LG_In1.csv};
                  \addplot[very thick, green, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcn_LG_In2.csv};
                % \legend{OPT};
                \end{axis}
            \end{tikzpicture}
      \end{subfigure}
...
\begin{subfigure}[!htb]{0.4\textwidth}
        ...
        \end{subfigure}
    % \end{center}
    \caption{Influence of the currents for the line to ground fault with a varying fault impedance in a two-converter case}
    \label{fig:Zf_2conv_LG}
  \end{figure}


\subsection{$R/X$ variation analysis}
The angle variation of the impedance of connection is applied to only $\un{Z}_{v1}$ in order to spot more clearly the differences in the distribution of currents between the sequences. For a line to line fault with ${Z}_x=0.1$, Fig. \ref{fig:Zf_2conv_LL} indicates that since $R_2/X_2$ is constant, the currents injected by VSC2 experience tiny changes. On the contrary, the currents injected by VSC1 change more significantly. The optimal strategy is to inject null currents in the positive sequence so that all current capability is employed to reduce the negative sequence voltage. A similar approach was found to be optimal in the one-converter case, as shown in Fig. \ref{fig:Zf_1conv_LL}. As a consequence, the positive sequence voltage in the OPT case becomes smaller than the one obtained for GCP and GCN, yet the reduction on the negative sequence voltage is more substantial, which causes the objective function to be reduced by more than 10\%. Out of the multiple faults analyzed in this paper, this is the one where differences in the objective function are larger.  
      ...
      \centering
          \begin{tikzpicture}[trim axis right,trim axis left]
              ...
              \begin{axis}[grid=major, xlabel={$R_1/X_1$}, ylabel={${I}^+$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={1}, axis line style = thick, ytick distance={0.1}, xmin=0.5, xmax=5, scaled y ticks=false, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=2}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_opt_LL_Ip1.csv};
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_opt_LL_Ip2.csv};
                  \addplot[very thick, cyan] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcp_LL_Ip1.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcp_LL_Ip2.csv};
                  \addplot[very thick, violet] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcn_LL_Ip1.csv};
                  \addplot[very thick, green, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcn_LL_Ip2.csv};
              \end{axis}
          \end{tikzpicture}
    \end{subfigure}
    \hspace{2.35cm}
    \begin{subfigure}[!htb]{0.12\textwidth}
        \centering
            \begin{tikzpicture}[trim axis right,trim axis left]
                \pgfplotsset{width=4.5cm, height=3.5cm}
                \begin{axis}[grid=major, xlabel={$R_1/X_1$}, ylabel={${I}^-$}, /pgf/number format/.cd, legend style={at={(0.98,0.15)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={1}, axis line style = thick, xmin=0.5, xmax=5, ytick distance = {0.1}, yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}, scaled x ticks=false, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3}]
                  \addplot[very thick, black] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_opt_LL_In1.csv};
                  \addplot[very thick, black, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_opt_LL_In2.csv};
                  \addplot[very thick, cyan] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcp_LL_In1.csv};
                  \addplot[very thick, red, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcp_LL_In2.csv};
                  \addplot[very thick, violet] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcn_LL_In1.csv};
                  \addplot[very thick, green, dashed] table[x=x, y=y, col sep=comma] {Data/Results_2conv_Zf_v2/res_gcn_LL_In2.csv};
                % \legend{OPT};
                \end{axis}
            \end{tikzpicture}
      \end{subfigure}
...
\begin{subfigure}[!htb]{0.4\textwidth}
        ...
        \end{subfigure}
    % \end{center}
    \caption{Influence of the currents for the line to line fault with a varying $R_1/X_1$ ratio and ${Z}_x=0.1$ in a two-converter case}
