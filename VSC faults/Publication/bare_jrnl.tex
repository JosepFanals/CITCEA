...
To do so, one component has to be expressed as a function of the other. This carries no extra complexity considering that phasor $\un{V}^-$ is fully known, and so the direction of $\un{I}^-$ has to be shifted $\frac{\pi}{2}$ from it. Making use of the relation such that:
\begin{equation}
  \tan(\alpha^-) = \frac{I^-_{im}}{I^-_{re}} \ ,
  \label{eq:tann}
\end{equation}
where current $I^-_{im}$ can be expressed as a function of $I^-_{re}$, for instance, and therefore there is only one unknown in each expression of \eqref{eq:Ifin}. Then, the resulting quadratic equations are solved for $I^-_{re}$ and the associated $I^-_{im}$ is found with \eqref{eq:tann}. 
...
When the prioritization applies to the negative sequence (GCN):

\begin{enumerate}
\item The absolute value of the negative sequence current is obtained from \eqref{eq:gc2}. Its direction in the complex plane is found by shifting it $+\frac{\pi}{2}$ with respect to $\un{V}^-$. Consequently, $I^-_{re}$ and $I^-_{im}$ are computed directly. The two unknowns in this case are $I^+_{re}$ and $I^+_{im}$. 
