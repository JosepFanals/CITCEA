...
For a single converter system as the one depicted in Fig. \ref{fig:sys_p1}, the fault impedance connected to the bus at voltage $\un{V}_f$ is responsible for identifying the fault. Explicitly, voltages and currents are related by
\begin{equation}
  \begin{pmatrix}
    \un{I}^{a}_1 \\
    \un{I}^{b}_1 \\
...
\begin{pmatrix}
  ...
  \end{pmatrix} ,
\end{equation}
where the admittance matrix has been split into two parts: $\mathbf{\un{Y}_v}$ represents the non-faulted admittance matrix of the system, whereas $\mathbf{\un{Y}_f}$ is constituted by the admittances that intervene in the fault. This way, the admittance matrices are defined as
\begin{equation}
  \scriptsize
