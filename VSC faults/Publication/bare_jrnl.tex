...
\begin{subfigure}[!htb]{0.4\textwidth}
  ...
  \end{figure}
Negative sequence currents are null, just like it could be foreseen for any balanced fault. The GC solution only injects imaginary positive sequence as real currents are not allowed by definition. The OPT solution achieves the optimal distribution of currents. The current capabilities of the converter are roughly equally attributed to the real and imaginary positivie sequence currents. Despite these differences, the GC solution is not significantly worse than the optimal. 
