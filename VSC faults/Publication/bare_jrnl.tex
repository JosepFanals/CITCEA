...
\begin{subfigure}[!htb]{0.4\textwidth}
  ...
  \end{figure}
Significantly different results are obtained in the case of a line to line fault, as shown in Figure \ref{fig:Zf_1conv_LL}. This unbalanced fault causes the optimal currents to be non-null for the imaginary part of the negative sequence. Even though the real current is kept at zero for GCP and GCN, their imaginary currents tend to approach the optimal value for large fault impedances. For low fault impedances, GCP prioritizes injecting current in the positive sequence whereas GCN does the same for the negative sequence, as expected. As a consequence, positive sequence voltages in the case of GCP are always superior to the ones obtained with GCN, and the contrary applies to the negative sequence voltages. Even though OPT looks for a balance between the positive and the negative sequence voltages, as seen from the optimization function in \eqref{eq:opt_1c}, it is able to reach higher positive sequence voltage values than GCP. This happens for moderate values of $\un{Z}_x$. In the end, there is an almost constant minimal difference between OPT and both GCP and GCN. All three objective functions are nearly the same. 
