...
\begin{subfigure}[!htb]{0.4\textwidth}
  ...
  \end{figure}
For large $R_1/X_1$ differences between the objective functions of OPT and GCP and GCN are acute due to the incapability of injecting real current. The OPT option achieves a larger positive sequence voltage than both GCP and GCN for all range of values. Although the negative sequence voltage obtained with OPT is initially larger, it is further reduced with increases in $R_1/X_1$, whereas this same voltage tends to grow for GCP and GCN. 
...
A two-converter case is proposed in order to spot the interaction between both and compare the performance of the optimization with the grid code rules. Fig. \ref{fig:sys_p2} shows a single-phase representation of the system under study. The converters are coupled to the grid by means of slightly different impedances $\un{Z}_{v1}$ and $\un{Z}_{v2}$. Table \ref{tab:param2} shows their values together with other input data. 
