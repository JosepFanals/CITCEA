...
Figure \ref{fig:sys_n} depicts a generic system formed by $n$ converters where the goal is to optimize the voltages at the point of common coupling. The AC side of VSCs is typically modeled as three-phase voltage sources in series with the filters' impedance of the form $\underline{Z}_z$. Figure XX shows the average model of a VSC, a simplified representation that excludes the switching of the semiconductors. In this work, emphasis is placed on the AC side. The control strategy directly adjusts the voltages so that currents can indirectly meet the references \cite{andani2018controller}. However, the approach followed here consists of treating them as current sources that inject a current of the form $\underline{I}_k$ into the system, i.e, into the grid denoted by $\mathbf{\underline{Y}_g}$. Such currents are precisely the variables to control in order to improve the voltages $\underline{V}_p$. 
...
\draw (0,0) to [C] (0,2);
\draw (1.0,0) to [isource] (1.0,2);
\node at (0.7,1.55) {${I}_{dc}$};
\node at (-0.3,1.3) {$C_{dc}$};
\draw (1,2) to [short, -*] (0,2);
\draw (1,0) to [short, -*] (0,0);
\draw (0,2) to [short, -*] (-1,2);
\draw (0,0) to [short, -*] (-1,0);
\draw[line width=0.4mm] (-1,2.5) to [short] (-1,-0.5);
\draw[line width=0.4mm] (-2,2.5) to [short] (-2,-0.5);
\draw[line width=0.4mm] (-2,2.5) to [short] (-1,2.5);
\draw[line width=0.4mm] (-2,-0.5) to [short] (-1,-0.5);
\node[rotate=00] at (-1.5,1.0) {DC syst.};

\end{circuitikz}
\caption{Average model of a VSC}
