...
Given a basic system as the one depicted in Figure \ref{fig:sys_p}, the goal is to optimize the voltages at the point of common coupling. The fault is symbolized by a generic fault impedance, which in reality can represent either a balanced or unbalanced fault.
...
\draw (-5,2) to [R, l=$\underline{Z}_{th}$] (-3,2);
\draw (-3.25,0) to [short] (-2.75,0);
\draw (-5.25,0) to [short] (-4.75,0);
\node at (-7,1.3) {PCC};
\node at (-9.5,1.3) {VSC};
\node at (-7,2.7) {$\underline{V}_c$};
