...
Figure \ref{fig:sys_c1} represents a third case regarding single converter systems, which deals with a hypothetical submarine cable modeled with its $\pi$ equivalent. 
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
The data selected to model the cable are extracted from \cite{cheah2017offshore} and adapted to per unit values. Table \ref{tab:cable} presents them.
\begin{table}\centering
  \caption{System parameters for the one-converter case}
  \begin{tabular}{ccc}
    \hline
...
% put table Cheah, plot of V and Z (like in previous work), extract results and comment some case
...
\begin{subfigure}[!htb]{0.4\textwidth}
          \centering
              \begin{tikzpicture}[trim axis right,trim axis left]
                  \pgfplotsset{width=9.4cm, height=3.51cm}
                  \begin{axis}[grid=major, xlabel={Distance (km)}, ylabel={$f$}, /pgf/number format/.cd, legend style={at={(0.98,0.17)},anchor=south east,legend columns=1, draw=none, inner sep=0pt,fill=gray!10}, xtick distance={10}, xmin=1, xmax=75, axis line style = thick, ytick distance = {0.02},  yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=3},]
